    ...
    return {
            ...
            newfeeds.forEach((newfeed) => {
                ...
                if (!(newfeed.deleted) && condition) {
                    ...
                    const cmts = comments.filter(comment => {
                        return comment.idPost === newfeed.id && !comment.sub && !comment.deleted
                    })
                    ...
                    <div class="newsfeed" data-index="${newfeed.id}">
                        <div class="newsfeed__info">
                            <div class="newsfeed__info-profile">
                                <img class="newsfeed__-profile-avt" src="${user.avatar}">
                            ...
                            <div class="newsfeed__info-setting more-dots">
                                ...
                                <ul class="newsfeed__info-setting-list" data-index=${newfeed.id}>
                                    ...
                                    <li 
                                    style="display: ${user.id === this.idUser ? 'flex' : 'none'}"
                                    ...
                                    <li style="display: ${user.id === this.idUser ? 'flex' : 'none'}" class="newsfeed__info-setting-item delete-post">
                                        <div class="newsfeed__info-setting-item__img">
                        ...
                        <div class="newsfeed__content">
                            ...
                            <img style="display: ${newfeed.bg ? 'block' : 'none'}" src="${newfeed.bg}" alt="" class="newsfeed__content-img">
                        </div>
    
                        <div class="newsfeed__respond">
                            <div title="${newfeed.like}" style="display: ${newfeed.like ? 'flex' : 'none'}" class="newsfeed__respond-react" >
                                <div class="newsfeed__respond-react-icon">
                                    <i class="fas fa-thumbs-up"></i>
                                </div>
                                <span class="newsfeed__respond-react-total">${compactNumber(newfeed.like)}</span>
                            </div>
                            <div style="display: ${cmts.length ? 'flex' : 'none'}" class="newsfeed__respond-right">
                                <span>${cmts.length} bình luận</span>
                                <span>${newfeed.share} lượt chia sẻ</span>
                            </div>
                        </div>
    
                        <ul data-index="${newfeed.id}" class="newsfeed__action" >
                            <li class="newsfeed__action-item ${users[this.idUser].postLiked.includes(newfeed.id.toString()) ? 'active' : ''} reaction">
                        ...
                        <div class="newsfeed__comment">
                            <div class="newsfeed__comment-user">
                                ...
                                <div class="newsfeed__comment-box">
                                    <input data-index=${newfeed.id} type="text" placeholder="Viết bình luận ..." class="newsfeed__comment-input">
                            ...
                            <div style="display: ${cmts.length ? 'block' : 'none'}" class="newsfeed__commented-box">
                                                        ...
                                                        cmtSubs.map((cmt2, index) => {
                                                            ...
                                                            <div class="commented-box__item-user sub">
                                                                ...
                                                                <div class="commented-box__item-info">
                                                                    ...
                                                                    <div data-id="${cmt2.id}"  data-index="${cmt.id}"  class="commented-box__item-reaction">
                                                                        <span class="commented-box__item-reaction--like ${users[this.idUser].cmtLiked.includes(cmt2.id.toString()) ? 'active' : ''}">Thích</span>
                                                                        •
                                                                        <span class="commented-box__item-reaction--respond">Phản hồi</span>
                                                                    </div>
                                                                    <div style="display : ${cmt2.like ? 'block' : 'none'}" class="commented-count-reaction">
                                                                        <i class="fas fa-thumbs-up"></i>
                                                                        <span>${compactNumber(cmt2.like)}</span>
                                                                    </div>
                                                                </div>
        ...
        handle(){
            ...
            newfeedLists.forEach((item) => {
                item.onclick = (e) => {
                    const likeBtn = e.target.closest('.newsfeed__action-item.reaction')
                    const cmtBtn = e.target.closest('.comment-action')
                    const likeCmtBtn = e.target.closest('.commented-box__item-reaction--like')
                    const respCmtBtn = e.target.closest('.commented-box__item-reaction--respond')
                    ...
                    checkLiked = (btn, action, arr, id) => {
                        ...
                        if (btn.classList.contains('active')) {
                            ...
                            users[this.idUser][action].forEach((item, index) => {
                                if (item == id) {
                                    users[this.idUser][action].splice(index, 1)
                                }
                            })
                        } else {
                    ...
                    if(showHideCmt) {
                        ...
                        if (boxCmt.classList.contains('active')){
                            ...
                            showHideCmt.querySelector('span').innerHTML = 'Thu gọn'
            ...
            postBtn.onclick = () => {
                ...
                if (_this.editMode) {
                    ...
                    if (editPost) {
                        ...
                        $('.post-box__btn.btn-disable').classList.remove('btn-active')
                    }
            ...
            window.onscroll = (e) => {
                ...
                if (myWall.style.display != 'none') {
                    ...
                    if (wallFriendsBottom < window.innerHeight) {
                        const heightSticky = window.innerHeight - wallFriends.offsetHeight
            ...
            //cmt
            window.onkeyup = (e) => {
                const responds = $$('.commented-box__item-reaction--respond.active')
                responds.forEach(resp => {
                    const inpBox = resp.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.parentElement.querySelector('.newsfeed__comment-input')
                    const idParent = Number.parseInt(resp.parentElement.dataset.index)
                ...
                const inpCmts = document.querySelectorAll('.newsfeed__comment-input')
